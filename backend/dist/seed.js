...
const seedDatabase = async () => {
    try {
        ...
        // Clear existing data
        await Hotel_1.default.deleteMany({});
        await Location_1.default.deleteMany({});
        console.log("Cleared existing data");
        // Insert locations
        const createdLocations = await Location_1.default.insertMany(locations);
        ...
        const hotelsWithEmbedding = hotels.map(async (hotel) => {
            ...
            const embedding = await (0, embeddings_1.generateEmbedding)(`${hotel.name} ${hotel.description} ${hotel.location} ${hotel.city} ${hotel.amenities.join(' ')} ${hotel.price}`);
